...
class EstimatorConfig(BaseModel):
    ...
    detector: str = 'opencv_lucaskanade'
...
class SpeedEstimator():
    ...
    def setup(self):
        ...
        self.target_imgs[0] = self.apply_camera_dewarp(self.target_imgs[0])
        ...
        with get_default_session_factory().get_db_with() as db:
            ...
            try:
                while self.running and self.get_next_image() and not stop_signal.is_set():
                    ...
                    if sum(len(i) for i in self.flow_groups) > 0:
                        # Extract events from bboxes
                        events = self.get_filter_new_events()
                        if len(events) > 0:
                            # Correlate events and vehicles
    ...
    def finish_tracked_vehicle(self, db: Session, vehicle: TrackedVehicle, read_time_delta=-1, MIN_EVENTS_FOR_VALID=30) -> bool: #TODO (Saving of flows)
        MIN_EVENTS_FOR_VALID = 5#np.floor(np.exp(-read_time_delta*0.75)*MIN_EVENTS_FOR_VALID)/3
    ...
    def get_next_image(self, num_to_read=1):
        ...
        return not any(img is None for img in self.target_imgs)
