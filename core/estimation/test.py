...
from core.estimation.estimator_schemas import VehicleDirection
...
class TestSpeedEstimator(unittest.TestCase):
    ...
    def await_estimator(self):
        self.estimator.await_completion()
    ...
    def test_speed_estimation_early_stop(self):
        ...
        # Run
        timer = time.time()
        signal = self.start_estimator()
        time.sleep(1) # Cut off early
        self.stop_estimator(signal)
        print(f"Time taken: {time.time() - timer}, length: {length}, slow down: {length / (time.time() - timer)}")
